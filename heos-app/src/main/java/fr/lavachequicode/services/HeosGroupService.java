...
public class HeosGroupService {
  ...
  protected void fetchFriendlyName(Group group) {
    if (group.getLeader() != null) {
      final GroupCurrentState leaderGroupState = heosStateService.getDeviceGroupState(UDN.valueOf(group.getLeader().getId()));
      group.setVolume(Integer.valueOf(leaderGroupState.getGroupVolume().getValue()));
      group.setFriendlyName(leaderGroupState.getGroupFriendlyName().getValue());
    }
  }
